...
#a.to_excel('神经网络预测.xlsx')
a = a.values



x0 = np.arange(len(a)).reshape(-1,1); y0 = a[:,1]  #提出训练样本数据
m1 = x0.max(axis=0); m2 = x0.min(axis=0)  #计算逐列最大值和最小值
bx0 = 2*(x0-m2)/(m1-m2)-1  #数据标准化
#构造并拟合模
md = MLPRegressor(solver='lbfgs',activation='relu',
     hidden_layer_sizes =17000,max_iter=300).fit(bx0, y0)
...
plt.plot(t, y0, linewidth = 2 ,color = 'red',label='原始数据')
#plt.plot(t, yh0.append(yh),linewidth = 2,color = 'dodgerblue',label='预测数据')
plt.legend()
plt.show()
